...
class CompGCNConvBasis(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_type, edge_norm=None, rel_embed=None):
        ...
        in_res = self.propagate(
            "add", self.in_index, x=x, edge_type=self.in_type, rel_embed=rel_embed, edge_norm=self.in_norm, mode="in"
        )
        loop_res = self.propagate(
            "add", self.loop_index, x=x, edge_type=self.loop_type, rel_embed=rel_embed, edge_norm=None, mode="loop"
        )
