...
class EdgeGraphsNBFNet(NBFNet):
    ...
    def __init__(
        ...
        self.edgegraph_model = MPNN(
            ...
            num_layers=edge_embed_num_layers,
...
class StructuralFeatureNBFNet(NBFNet):
    ...
    def forward(self, data, batch):
        ...
        data.x = self.up_node_emb.weight.repeat(
            (data.num_nodes, 1)
        )  # (num_nodes, input_dim)
        data.x = None
...
class PriceFeatureNBFNet(NBFNet):
    def __init__(
        ...
        # Optional: Initialize weights for better stability
        for layer in self.phi:
            if isinstance(layer, nn.Linear):
                nn.init.xavier_uniform_(layer.weight)
                nn.init.zeros_(layer.bias)

        # Optional: Define a normalizer if needed
        self.price_normalizer = nn.BatchNorm1d(1)  # You can choose other normalizations

    def forward(self, data, batch):
        # Extract and reshape the price feature
        x = data.x_price.unsqueeze(-1)  # Shape: (num_nodes, 1)

        # **Step 1: Replace Inf and NaN Values**

        # Replace Inf with finite numbers and NaN with zero
        # posinf -> 1e10, neginf -> -1e10, nan -> 0.0
        x_clean = torch.nan_to_num(x, nan=0.0, posinf=1e10, neginf=-1e10)

        # Optional: Log the number of replacements for debugging
        num_pos_inf = torch.isinf(x).sum().item()
        num_neg_inf = torch.isinf(x).sum().item()
        num_nan = torch.isnan(x).sum().item()

        # if num_pos_inf > 0 or num_neg_inf > 0 or num_nan > 0:
        #     print(f"Replaced {num_pos_inf} Inf, {num_neg_inf} -Inf, and {num_nan} NaN values in x_price.")

        # **Step 2: Normalize the Data (Optional but Recommended)**
        # Normalizing can help in stabilizing the training process
        x_normalized = self.price_normalizer(x_clean)

        # **Step 3: Pass Through the phi Network**
        x_projected = self.phi(x_normalized)

        # **Step 4: Assign the Processed Features Back to data.x**
        data.x = x_projected  # Shape: (num_nodes, input_dim)

        # **Step 5: Proceed with the Base Class Forward Pass**
        return super().forward(data, batch)
...
class NormalizedFeatureNBFNet(NBFNet):
    ...
    def forward(self, data, batch):
        x = self.phi(
            data.x_normalized.unsqueeze(-1)
        )  # (num_feat, num_nodes) -> (num_feat, num_nodes, input_dim)
