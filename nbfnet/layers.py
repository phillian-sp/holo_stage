...
class EdgeRGCNConv(MessagePassing):
    ...
    def __init__(
        ...
        edgegraph_mlp: Optional[nn.Module] = None,
    ):
    ...
    def message(
        ...
        if self.cat_method == "cat":
            ...
            raise ValueError("Unknown concatenation method `%s`" % self.cat_method)
    ...
    def aggregate(self, input, edge_weight, index, edge_type, dim_size):
        ...
        output = torch.zeros(
            (input.size(0), dim_size, input.size(2)), device=input.device
        )
