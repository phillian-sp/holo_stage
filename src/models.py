import torch
import torch.nn as nn
from torch_geometric.nn import GINEConv, GCNConv
from torch_geometric.nn.pool import global_add_pool
from dataclasses import dataclass, field
from torch_geometric.utils import to_dense_adj

from .nbfnet.nbf_model import NBFNet, NBFNetConfig
from .rgcn.rgcn_model import RGCN, RGCNConfig
from .compgcn.compgcn_model import CompGCN, CompGCNConfig
from .holo import Holo, PowerMethod


class MPNN(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layers, edge_model, edge_dim):
        super().__init__()

        self.convs = torch.nn.ModuleList()
        self.bns = torch.nn.ModuleList()
        for _ in range(num_layers):
            if edge_model == "GINEConv":
                mlp = torch.nn.Sequential(
                    torch.nn.Linear(input_dim, hidden_dim),
                    torch.nn.ReLU(),
                    torch.nn.Linear(hidden_dim, hidden_dim),
                )
                self.convs.append(GINEConv(nn=mlp, edge_dim=edge_dim))
            elif edge_model == "GCNConv":
                self.convs.append(GCNConv(input_dim, hidden_dim))
            self.bns.append(torch.nn.BatchNorm1d(hidden_dim))
            input_dim = hidden_dim

    def forward(self, x, edge_index, edge_attr):
        for conv, bn in zip(self.convs, self.bns):
            x = conv(x, edge_index, edge_attr)
            x = bn(x).relu()

        return x


@dataclass
class EdgeGraphsModelConfig:
    edge_embed_dim: int = 256
    edge_embed_num_layers: int = 1
    edge_model: str = "GINEConv"
    use_p_value: int = 1

    final_model: str = "nbf"
    nbf: NBFNetConfig = field(default_factory=NBFNetConfig)
    rgcn: RGCNConfig = field(default_factory=RGCNConfig)
    compgcn: CompGCNConfig = field(default_factory=CompGCNConfig)


class EdgeGraphsModel(nn.Module):
    def __init__(
        self,
        num_relation,
        cfg: EdgeGraphsModelConfig,
    ):
        super().__init__()
        self.edge_embed_dim = cfg.edge_embed_dim
        # Create an instance of NBFNet as a member variable
        if cfg.final_model == "nbf":
            self.model = NBFNet(num_relation, cfg.edge_embed_dim, cfg.nbf)
        elif cfg.final_model == "rgcn":
            self.model = RGCN(num_relation, cfg.edge_embed_dim, cfg.rgcn)
        elif cfg.final_model == "compgcn":
            self.model = CompGCN(num_relation, cfg.edge_embed_dim, cfg.compgcn)
        else:
            raise ValueError(f"Invalid final model: {cfg.final_model}")
        if cfg.use_p_value:
            edge_dim = 2
        else:
            edge_dim = 1
        self.edgegraph_model = MPNN(
            input_dim=1,
            hidden_dim=cfg.edge_embed_dim,
            num_layers=cfg.edge_embed_num_layers,
            edge_model=cfg.edge_model,
            edge_dim=edge_dim,
        )
        self.up_emb = torch.nn.Embedding(1, cfg.edge_embed_dim)  # same embedding for all user product edges

        self.edge_model = cfg.edge_model
        self.use_p_value = cfg.use_p_value

        # print number of parameters in self.model
        num_params = sum(p.numel() for p in self.model.parameters())
        print(f"Number of parameters in self.model: {num_params}")

    def forward(self, data, batch):
        if data.edgegraph_edge_attr.dim() == 1:
            data.edgegraph_edge_attr = data.edgegraph_edge_attr.unsqueeze(-1)
        if self.edge_model == "GCNConv":
            data.edgegraph_edge_attr = data.edgegraph_edge_attr[:, 0:1]
        h = self.edgegraph_model(data.edgegraph_x, data.edgegraph_edge_index, data.edgegraph_edge_attr)
        edgegraph_reprs = global_add_pool(h, data.edgegraph2ppedge)

        num_up_edges = data.edge_index.size(-1) - edgegraph_reprs.size(0)
        upgraph_emb = self.up_emb.weight.repeat((num_up_edges, 1))
        edge_embeddings = torch.vstack([upgraph_emb, edgegraph_reprs])

        data.edge_embeddings = edge_embeddings
        data.x = None
        # Use the NBFNet instance instead of calling super()
        return self.model.forward(data, batch)


@dataclass
class HoloModelConfig:
    edge_embed_dim: int = 256
    edge_embed_num_layers: int = 1
    edge_model: str = "GINEConv"
    use_p_value: int = 1
    n_breakings: int = 8
    power: int = 2

    model1: str = "rgcn"
    model2: str = "rgcn"
    nbf1: NBFNetConfig = field(default_factory=NBFNetConfig)
    rgcn1: RGCNConfig = field(default_factory=RGCNConfig)
    compgcn1: CompGCNConfig = field(default_factory=CompGCNConfig)

    nbf2: NBFNetConfig = field(default_factory=NBFNetConfig)
    rgcn2: RGCNConfig = field(default_factory=RGCNConfig)
    compgcn2: CompGCNConfig = field(default_factory=CompGCNConfig)


class HoloModel(nn.Module):
    def __init__(
        self,
        num_relation,
        cfg: HoloModelConfig,
    ):
        super().__init__()
        self.edge_embed_dim = cfg.edge_embed_dim
        # Create an instance of NBFNet as a member variable
        if cfg.model1 == "nbf":
            self.model1 = NBFNet(num_relation, cfg.edge_embed_dim, cfg.nbf1)
        elif cfg.model1 == "rgcn":
            self.model1 = RGCN(num_relation, cfg.edge_embed_dim, cfg.rgcn1, return_emb=True)
        elif cfg.model1 == "compgcn":
            self.model1 = CompGCN(num_relation, cfg.edge_embed_dim, cfg.compgcn1, return_emb=True)
        else:
            raise ValueError(f"Invalid final model1: {cfg.model1}")
        if cfg.model2 == "nbf":
            self.model2 = NBFNet(num_relation, cfg.edge_embed_dim, cfg.nbf2)
        elif cfg.model2 == "rgcn":
            self.model2 = RGCN(num_relation, cfg.edge_embed_dim, cfg.rgcn2)
        elif cfg.model2 == "compgcn":
            self.model2 = CompGCN(num_relation, cfg.edge_embed_dim, cfg.compgcn2)
        else:
            raise ValueError(f"Invalid final model2: {cfg.model2}")

        if cfg.use_p_value:
            edge_dim = 2
        else:
            edge_dim = 1
        self.edgegraph_model = MPNN(
            input_dim=1,
            hidden_dim=cfg.edge_embed_dim,
            num_layers=cfg.edge_embed_num_layers,
            edge_model=cfg.edge_model,
            edge_dim=edge_dim,
        )
        symmetric_breaking_model = PowerMethod(cfg.power, self.model1.output_dim + 1)
        self.holo = Holo(n_breakings=cfg.n_breakings, symmetric_breaking_model=symmetric_breaking_model)

        self.up_emb = torch.nn.Embedding(1, cfg.edge_embed_dim)  # same embedding for all user product edges

        self.edge_model = cfg.edge_model
        self.use_p_value = cfg.use_p_value

        # print number of parameters in self.model
        num_params = sum(p.numel() for p in self.model1.parameters()) + sum(
            p.numel() for p in self.model2.parameters()
        )
        print(f"Number of parameters in self.model: {num_params}")

    def forward(self, data, batch):
        if data.edgegraph_edge_attr.dim() == 1:
            data.edgegraph_edge_attr = data.edgegraph_edge_attr.unsqueeze(-1)
        if self.edge_model == "GCNConv":
            data.edgegraph_edge_attr = data.edgegraph_edge_attr[:, 0:1]
        h = self.edgegraph_model(data.edgegraph_x, data.edgegraph_edge_index, data.edgegraph_edge_attr)
        edgegraph_reprs = global_add_pool(h, data.edgegraph2ppedge)

        num_up_edges = data.edge_index.size(-1) - edgegraph_reprs.size(0)
        upgraph_emb = self.up_emb.weight.repeat((num_up_edges, 1))
        edge_embeddings = torch.vstack([upgraph_emb, edgegraph_reprs])

        data.edge_embeddings = edge_embeddings
        data.x = None
        # return self.model.forward(data, batch)

        # forward pass through model1
        x = self.model1.forward_emb(data)

        # get adjacency matrix for model2
        adj_t = to_dense_adj(data.edge_index, max_num_nodes=data.num_nodes)[0]
        data.x = self.holo(x.squeeze(0), adj_t).unsqueeze(0)
        # data.x = x

        # forward pass through model2
        return self.model2.forward(data, batch)
